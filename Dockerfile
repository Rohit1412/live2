...
RUN apt-get update && \
  apt-get install -y \
    ...
    pkg-config openssl libssl-dev yasm nasm \
...
RUN cd /tmp/build/nginx-${NGINX_VERSION} && \
    ...
    CFLAGS="-Wno-error -Wno-implicit-fallthrough" ./configure \
...
RUN apt-get update && \
	apt-get install -y \
		...
		libvpx5 libx264-dev libx265-dev \
		procps net-tools iproute2 procps watch && \
    rm -rf /var/lib/apt/lists/*
